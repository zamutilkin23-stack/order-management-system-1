...
export default function OrderStatusManager({
  ...
  const [editingItem, setEditingItem] = useState<{ itemId: number; value: string } | null>(null);
  ...
  const filteredOrders = orders.filter(order => {
    if (statusFilter === 'active') return order.status !== 'completed' && order.status !== 'shipped';
    if (statusFilter === 'completed') return order.status === 'completed';
    return true;
  });

  const handleUpdateQuantity = (itemId: number, newValue: number) => {
    if (selectedOrder) {
      onUpdateItem(selectedOrder.id, itemId, newValue);
      setEditingItem(null);
      setTimeout(() => {
        onRefresh();
      }, 500);
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Выполнение заявок</CardTitle>
              <CardDescription>Внесение выполненного объема работ</CardDescription>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {selectedOrder.items?.map((item) => {
                  ...
                  const progress = item.quantity_required > 0 
                        ...
                        {isEditing ? (
                          <>
                            <Input
                              ...
                              onChange={(e) => setEditingItem({ itemId: item.id, value: e.target.value })}
                              className="flex-1"
                              autoFocus
                            />
                            <Button
                              size="sm"
                              onClick={() => handleUpdateQuantity(item.id, Number(editingItem.value))}
                            >
                          ...
                          <>
                            <Label className="text-sm text-gray-600 flex-1">
                              Выполнено: {item.quantity_completed} шт
                            </Label>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => setEditingItem({ itemId: item.id, value: String(item.quantity_completed) })}
                            >
